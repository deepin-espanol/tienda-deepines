...
void CircleProgressAnimatePad::setMiddleText(QString &str)
{
    middleText = str;
}
...
void CircleProgressAnimatePad::paintEvent(QPaintEvent *event)
{
    ...
    const int offset = (int)(-percent * 16 * 360);
    ...
    } else if (!isAnimateStarted && middleTextVisible) {//draw text
        QString text;
        if (middleText != nullptr && middleText != "") {
             text = middleText;
        } else {
            text = QString::number(m_currentValue);
        }
        QFont font;
...
void CircleProgressAnimatePad::setMiddleTextVisible(bool visible)
{
    middleTextVisible = visible;
}
