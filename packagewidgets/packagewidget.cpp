...
#include "packagewidgets/headerbar.h"
#include "packagewidgets/datafields.h"
...
public:
    ...
    {
        QVBoxLayout *mLayout = new QVBoxLayout(Form);
        mLayout->setMargin(0);
        mLayout->setSpacing(0);
        lay = new QVBoxLayout;
        lay->setContentsMargins(5, -1, 12, -1);

        pushButton = new SplittedButton;
        pushButton->setFixedHeight(40);
        bar = new HeaderBar(Form);
        bar->setFixedHeight(200);
        bar->setWidget(pushButton);

        mLayout->addWidget(bar);
        mLayout->addLayout(lay);

        description_container = new QFrame();
        ...
        lay->addWidget(description_container);
        //verticalLayout_6->addLayout(horizontalLayout);
        //verticalLayout_6->addWidget();
        ...
        lay->addWidget(screenShotWidget);
        ...
        lay->addWidget(dependencies_container);
        ...
        lay->addWidget(installedFiles_container);
        ...
        lay->addWidget(requiredBy_container);
        ...
        lay->addWidget(enhances_container);
        ...
        lay->addWidget(enhancedBy_container);
        ...
        lay->addWidget(recommends_container);
        ...
        lay->addWidget(suggests_container);
        ...
        lay->addWidget(provides_container);
...
PackageWidget::PackageWidget(QWidget *parent) : QScrollArea(parent)
{
    ...
    m_widget->setContentsMargins(0, 0, 0, 5);
...
void PackageWidget::reloadUI()
{
    ...
    //std::cout << oldPkg->controlField("Package").toLocal8Bit().data() << std::endl;
    ...
    //To avoid a UI bug that I seen, recalculate the widget's content (the button) 's pos, else the button might not be at the good position!
    ui->bar->checkSizes();
...
void PackageWidget::resizeEvent(QResizeEvent *e)
{
